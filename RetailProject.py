{
  "nbformat": 4,
  "nbformat_minor": 0,
  "metadata": {
    "colab": {
      "provenance": [],
      "mount_file_id": "1L6PluvrX7OFF4OAc7Lwk460779C-C-OX",
      "authorship_tag": "ABX9TyN8ROMJEo2+efQfQcRVgWnS",
      "include_colab_link": true
    },
    "kernelspec": {
      "name": "python3",
      "display_name": "Python 3"
    },
    "language_info": {
      "name": "python"
    }
  },
  "cells": [
    {
      "cell_type": "markdown",
      "metadata": {
        "id": "view-in-github",
        "colab_type": "text"
      },
      "source": [
        "<a href=\"https://colab.research.google.com/github/LoveDatax/RetailProject/blob/main/RetailProject.py\" target=\"_parent\"><img src=\"https://colab.research.google.com/assets/colab-badge.svg\" alt=\"Open In Colab\"/></a>"
      ]
    },
    {
      "cell_type": "code",
      "execution_count": null,
      "metadata": {
        "id": "pT46cvS3K24O"
      },
      "outputs": [],
      "source": [
        "from pyspark.sql import SparkSession\n",
        "from pyspark.sql.functions import col, lower, upper, trim, to_date, date_format, sum, regexp_replace, count, avg\n",
        "from pyspark.sql.types import DecimalType\n",
        "spark=SparkSession.builder\\\n",
        ".appName('Retail data project')\\\n",
        ".getOrCreate()"
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "retails=spark.read.csv('/content/drive/MyDrive/Retail_Transactions_Dataset.csv',\n",
        "                       header=True,\n",
        "                       inferSchema=True,\n",
        "                       sep=',',\n",
        "                       escape='\"',\n",
        "                       quote='\"',\n",
        "                       multiLine=True,\n",
        "                       mode='PERMISSIVE')\n",
        "retails.printSchema()"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "id": "dr3WUanhXgwy",
        "outputId": "fb9f9711-3b67-469a-db19-9959d8cc85f8"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "root\n",
            " |-- Transaction_ID: integer (nullable = true)\n",
            " |-- Date: timestamp (nullable = true)\n",
            " |-- Customer_Name: string (nullable = true)\n",
            " |-- Product: string (nullable = true)\n",
            " |-- Total_Items: integer (nullable = true)\n",
            " |-- Total_Cost: double (nullable = true)\n",
            " |-- Payment_Method: string (nullable = true)\n",
            " |-- City: string (nullable = true)\n",
            " |-- Store_Type: string (nullable = true)\n",
            " |-- Discount_Applied: boolean (nullable = true)\n",
            " |-- Customer_Category: string (nullable = true)\n",
            " |-- Season: string (nullable = true)\n",
            " |-- Promotion: string (nullable = true)\n",
            "\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "retails.show(10, truncate=False)"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "id": "JQ5q56QLOQMA",
        "outputId": "9f238105-cdc8-4970-82dc-3d426020636e"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "+--------------+-------------------+-----------------+-------------------------------------------------------------+-----------+----------+--------------+-------------+-----------------+----------------+-----------------+------+--------------------------+\n",
            "|Transaction_ID|Date               |Customer_Name    |Product                                                      |Total_Items|Total_Cost|Payment_Method|City         |Store_Type       |Discount_Applied|Customer_Category|Season|Promotion                 |\n",
            "+--------------+-------------------+-----------------+-------------------------------------------------------------+-----------+----------+--------------+-------------+-----------------+----------------+-----------------+------+--------------------------+\n",
            "|1000000000    |2022-01-21 06:27:29|Stacey Price     |['Ketchup', 'Shaving Cream', 'Light Bulbs']                  |3          |71.65     |Mobile Payment|Los Angeles  |Warehouse Club   |true            |Homemaker        |Winter|None                      |\n",
            "|1000000001    |2023-03-01 13:01:21|Michelle Carlson |['Ice Cream', 'Milk', 'Olive Oil', 'Bread', 'Potatoes']      |2          |25.93     |Cash          |San Francisco|Specialty Store  |true            |Professional     |Fall  |BOGO (Buy One Get One)    |\n",
            "|1000000002    |2024-03-21 15:37:04|Lisa Graves      |['Spinach']                                                  |6          |41.49     |Credit Card   |Houston      |Department Store |true            |Professional     |Winter|None                      |\n",
            "|1000000003    |2020-10-31 09:59:47|Mrs. Patricia May|['Tissues', 'Mustard']                                       |1          |39.34     |Mobile Payment|Chicago      |Pharmacy         |true            |Homemaker        |Spring|None                      |\n",
            "|1000000004    |2020-12-10 00:59:59|Susan Mitchell   |['Dish Soap']                                                |10         |16.42     |Debit Card    |Houston      |Specialty Store  |false           |Young Adult      |Winter|Discount on Selected Items|\n",
            "|1000000005    |2021-10-07 12:37:26|Joshua Frazier   |['Toothpaste', 'Chicken']                                    |3          |72.24     |Cash          |Houston      |Supermarket      |true            |Retiree          |Spring|Discount on Selected Items|\n",
            "|1000000006    |2023-01-08 10:40:03|Victoria Garrett |['Honey', 'BBQ Sauce', 'Soda', 'Olive Oil', 'Garden Hose']   |4          |5.28      |Cash          |Boston       |Specialty Store  |false           |Student          |Summer|Discount on Selected Items|\n",
            "|1000000007    |2020-09-03 12:39:59|Sydney Waller    |['Syrup', 'Trash Cans', 'Pancake Mix', 'Water', 'Mayonnaise']|5          |21.77     |Debit Card    |Chicago      |Specialty Store  |false           |Young Adult      |Winter|Discount on Selected Items|\n",
            "|1000000008    |2021-04-05 06:32:18|Kimberly Morgan  |['Insect Repellent']                                         |4          |55.25     |Mobile Payment|Los Angeles  |Warehouse Club   |false           |Homemaker        |Fall  |None                      |\n",
            "|1000000009    |2021-07-08 10:08:59|Lori Conway      |['Soap', 'Baby Wipes', 'Soda']                               |7          |31.21     |Mobile Payment|Boston       |Convenience Store|true            |Young Adult      |Winter|None                      |\n",
            "+--------------+-------------------+-----------------+-------------------------------------------------------------+-----------+----------+--------------+-------------+-----------------+----------------+-----------------+------+--------------------------+\n",
            "only showing top 10 rows\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "retails_processed=retails\\\n",
        ".select(col(\"Transaction_ID\").alias(\"transaction_id\"),\n",
        "    col(\"Date\").alias(\"transaction_ts\"),\n",
        "    col(\"Customer_Name\").alias(\"customer_name\"),\n",
        "    col(\"Product\").alias(\"product\"),\n",
        "    col(\"Total_Items\").alias(\"total_items\"),\n",
        "    col(\"Total_Cost\").alias(\"total_cost\"),\n",
        "    col(\"Payment_Method\").alias(\"payment_method\"),\n",
        "    col(\"City\").alias(\"city\"),\n",
        "    col(\"Store_Type\").alias(\"store_type\"),\n",
        "    col(\"Discount_Applied\").alias(\"discount_applied\"),\n",
        "    col(\"Customer_Category\").alias(\"customer_category\"),\n",
        "    col(\"Season\").alias(\"season\"),\n",
        "    col(\"Promotion\").alias(\"promotion\"))\\\n",
        ".filter(col('transaction_id').isNotNull())\\\n",
        ".filter(col('transaction_ts').isNotNull())\\\n",
        ".filter(col('total_items') > 0)\\\n",
        ".filter(col('total_cost') >= 0)\\\n",
        ".withColumn('total_cost', regexp_replace(col('total_cost'), '[^0-9.]', '').cast(DecimalType(12,2)))\\\n",
        ".withColumn('customer_name', upper(trim(col('customer_name'))))\\\n",
        ".withColumn('payment_method', upper(trim(col('payment_method'))))\\\n",
        ".withColumn('city', upper(trim(col('city'))))\\\n",
        ".withColumn('store_type', upper(trim(col('store_type'))))\\\n",
        ".withColumn('customer_category', upper(trim(col('customer_category'))))\\\n",
        ".withColumn('season', upper(trim(col('season'))))\\\n",
        ".withColumn('promotion', upper(trim(col('promotion'))))\\\n",
        ".withColumn('transaction_time', date_format(col('transaction_ts'),'HH:mm:ss'))\\\n",
        ".withColumn('transaction_date', to_date(col('transaction_ts')))\\\n",
        ".drop('transaction_ts')\\\n",
        ".dropDuplicates()"
      ],
      "metadata": {
        "id": "kdCPJSH1yvRF"
      },
      "execution_count": null,
      "outputs": []
    },
    {
      "cell_type": "code",
      "source": [
        "retails_processed.show(20, truncate=False)"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "id": "1afamzn1Bzlg",
        "outputId": "e218a76d-deb7-4b3f-9868-795c32e48d89"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "+--------------+-------------------+----------------------------------------------------------------------------+-----------+----------+--------------+-------------+-----------------+----------------+-----------------+------+--------------------------+----------------+----------------+\n",
            "|transaction_id|customer_name      |product                                                                     |total_items|total_cost|payment_method|city         |store_type       |discount_applied|customer_category|season|promotion                 |transaction_time|transaction_date|\n",
            "+--------------+-------------------+----------------------------------------------------------------------------+-----------+----------+--------------+-------------+-----------------+----------------+-----------------+------+--------------------------+----------------+----------------+\n",
            "|1000000690    |VICTORIA BROWN     |['Soap', 'Toilet Paper', 'Peanut Butter', 'Vinegar', 'Trash Cans']          |2          |56.31     |MOBILE PAYMENT|CHICAGO      |CONVENIENCE STORE|false           |SENIOR CITIZEN   |WINTER|DISCOUNT ON SELECTED ITEMS|08:36:05        |2020-02-20      |\n",
            "|1000000721    |MARY HENDRICKS     |['Trash Cans', 'Cereal', 'Broom']                                           |3          |57.58     |MOBILE PAYMENT|HOUSTON      |CONVENIENCE STORE|true            |MIDDLE-AGED      |FALL  |NONE                      |01:43:37        |2020-07-27      |\n",
            "|1000000864    |ANTHONY HAYNES     |['Olive Oil', 'Toothpaste']                                                 |10         |23.35     |DEBIT CARD    |LOS ANGELES  |DEPARTMENT STORE |false           |YOUNG ADULT      |WINTER|BOGO (BUY ONE GET ONE)    |02:55:51        |2022-01-18      |\n",
            "|1000000967    |SARA RIVERS        |['Toothpaste', 'Cleaning Spray', 'Jam', 'Air Freshener', 'Insect Repellent']|8          |82.80     |CREDIT CARD   |ATLANTA      |SUPERMARKET      |false           |HOMEMAKER        |WINTER|NONE                      |14:54:34        |2023-04-05      |\n",
            "|1000001452    |HOLLY SMITH        |['Tuna', 'Cereal Bars', 'Paper Towels', 'Onions']                           |5          |41.62     |MOBILE PAYMENT|DALLAS       |WAREHOUSE CLUB   |false           |YOUNG ADULT      |FALL  |BOGO (BUY ONE GET ONE)    |17:50:56        |2021-07-23      |\n",
            "|1000001603    |BRADLEY BARRETT    |['Mayonnaise', 'Hand Sanitizer', 'Tuna', 'Olive Oil', 'Bath Towels']        |2          |22.02     |CASH          |CHICAGO      |SUPERMARKET      |false           |YOUNG ADULT      |SUMMER|NONE                      |11:41:57        |2020-12-27      |\n",
            "|1000001702    |MARGARET BOYD      |['Laundry Detergent', 'Toothpaste', 'Vacuum Cleaner', 'Toothbrush']         |6          |67.84     |CREDIT CARD   |CHICAGO      |SUPERMARKET      |true            |YOUNG ADULT      |WINTER|NONE                      |16:45:30        |2021-08-29      |\n",
            "|1000001895    |REGINALD SINGH     |['Plant Fertilizer', 'Dish Soap', 'Cheese']                                 |9          |84.25     |CREDIT CARD   |CHICAGO      |CONVENIENCE STORE|true            |PROFESSIONAL     |WINTER|BOGO (BUY ONE GET ONE)    |14:36:29        |2023-02-11      |\n",
            "|1000001914    |DWAYNE ARROYO      |['Chips', 'Chicken', 'Cleaning Rags', 'Dustpan']                            |10         |97.50     |CREDIT CARD   |HOUSTON      |PHARMACY         |true            |HOMEMAKER        |FALL  |DISCOUNT ON SELECTED ITEMS|02:01:32        |2020-02-13      |\n",
            "|1000002040    |KAYLA HINES        |['Apple', 'Beef', 'Spinach', 'Milk']                                        |5          |35.11     |MOBILE PAYMENT|MIAMI        |DEPARTMENT STORE |false           |TEENAGER         |SPRING|BOGO (BUY ONE GET ONE)    |02:50:33        |2021-04-23      |\n",
            "|1000002247    |BRADLEY MULLEN     |['Lawn Mower', 'Cereal Bars', 'Soap', 'Cleaning Rags', 'Coffee']            |3          |8.04      |DEBIT CARD    |ATLANTA      |PHARMACY         |true            |STUDENT          |FALL  |NONE                      |05:12:31        |2020-11-25      |\n",
            "|1000002360    |NICHOLAS DEAN      |['Bath Towels', 'Diapers']                                                  |1          |95.15     |CREDIT CARD   |SAN FRANCISCO|SUPERMARKET      |false           |SENIOR CITIZEN   |WINTER|BOGO (BUY ONE GET ONE)    |05:18:48        |2020-01-24      |\n",
            "|1000002369    |JOSEPH BLACK       |['Ice Cream', 'Shaving Cream', 'Vinegar', 'Banana']                         |3          |15.45     |DEBIT CARD    |SEATTLE      |DEPARTMENT STORE |true            |PROFESSIONAL     |FALL  |NONE                      |22:55:51        |2022-05-28      |\n",
            "|1000002406    |PRISCILLA CLAY     |['Water', 'Tea', 'Pasta', 'Tomatoes', 'Banana']                             |9          |19.59     |MOBILE PAYMENT|LOS ANGELES  |SUPERMARKET      |false           |SENIOR CITIZEN   |SUMMER|BOGO (BUY ONE GET ONE)    |08:46:36        |2022-09-13      |\n",
            "|1000002410    |MONIQUE JACKSON    |['Pickles']                                                                 |6          |71.10     |CASH          |ATLANTA      |SPECIALTY STORE  |false           |STUDENT          |SPRING|NONE                      |06:37:25        |2021-09-11      |\n",
            "|1000002624    |ERIC BROOKS        |['Chips', 'Tomatoes', 'Cleaning Rags']                                      |7          |95.55     |DEBIT CARD    |ATLANTA      |DEPARTMENT STORE |false           |STUDENT          |SUMMER|DISCOUNT ON SELECTED ITEMS|12:47:58        |2021-03-13      |\n",
            "|1000002630    |JACQUELINE FRANKLIN|['Mayonnaise', 'BBQ Sauce', 'Paper Towels', 'Power Strips']                 |2          |10.68     |CREDIT CARD   |LOS ANGELES  |SPECIALTY STORE  |false           |MIDDLE-AGED      |WINTER|BOGO (BUY ONE GET ONE)    |15:06:31        |2021-02-07      |\n",
            "|1000002808    |AMBER SMITH        |['Mustard', 'Dish Soap']                                                    |8          |58.34     |CREDIT CARD   |SAN FRANCISCO|DEPARTMENT STORE |false           |TEENAGER         |WINTER|BOGO (BUY ONE GET ONE)    |20:46:26        |2023-07-10      |\n",
            "|1000002827    |SHARI MARTINEZ     |['Milk', 'Milk', 'Canned Soup', 'Paper Towels', 'Orange']                   |2          |10.76     |DEBIT CARD    |HOUSTON      |SUPERMARKET      |true            |MIDDLE-AGED      |SPRING|DISCOUNT ON SELECTED ITEMS|14:06:10        |2022-08-14      |\n",
            "|1000002939    |MICHAEL KENT       |['Plant Fertilizer', 'Apple', 'Ice Cream', 'Laundry Detergent']             |6          |70.06     |CASH          |HOUSTON      |DEPARTMENT STORE |false           |HOMEMAKER        |WINTER|DISCOUNT ON SELECTED ITEMS|20:33:50        |2020-03-01      |\n",
            "+--------------+-------------------+----------------------------------------------------------------------------+-----------+----------+--------------+-------------+-----------------+----------------+-----------------+------+--------------------------+----------------+----------------+\n",
            "only showing top 20 rows\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "city_revenue = retails_processed\\\n",
        "    .groupBy('city')\\\n",
        "    .agg(sum('total_cost').alias('total_revenue'))\\\n",
        "    .orderBy(col('total_revenue').desc())\n",
        "\n",
        "city_revenue.show()"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "id": "uanj1daHVl0Q",
        "outputId": "2db904d7-923c-4143-dd17-824add3ce698"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "+-------------+-------------+\n",
            "|         city|total_revenue|\n",
            "+-------------+-------------+\n",
            "|       DALLAS|   5277111.53|\n",
            "|       BOSTON|   5263307.96|\n",
            "|      CHICAGO|   5263187.45|\n",
            "|     NEW YORK|   5252469.92|\n",
            "|      HOUSTON|   5247054.78|\n",
            "|SAN FRANCISCO|   5241099.86|\n",
            "|        MIAMI|   5240498.44|\n",
            "|      SEATTLE|   5235365.43|\n",
            "|  LOS ANGELES|   5232393.19|\n",
            "|      ATLANTA|   5202731.84|\n",
            "+-------------+-------------+\n",
            "\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "store_type_revenue = retails_processed\\\n",
        "    .groupBy('store_type')\\\n",
        "    .agg(sum('total_cost').alias('total_revenue')) \\\n",
        "    .orderBy(col('total_revenue').desc())\n",
        "\n",
        "store_type_revenue.show()"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "id": "oM2yRF6Najtd",
        "outputId": "9544a5d9-f66e-427a-9448-556e3d8b6fec"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "+-----------------+-------------+\n",
            "|       store_type|total_revenue|\n",
            "+-----------------+-------------+\n",
            "|         PHARMACY|   8766679.01|\n",
            "|      SUPERMARKET|   8763455.21|\n",
            "|   WAREHOUSE CLUB|   8760029.03|\n",
            "|CONVENIENCE STORE|   8731901.36|\n",
            "| DEPARTMENT STORE|   8731555.57|\n",
            "|  SPECIALTY STORE|   8701600.22|\n",
            "+-----------------+-------------+\n",
            "\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "promotion_performance = retails_processed\\\n",
        "    .filter(col('promotion').isNotNull())\\\n",
        "    .groupBy('promotion')\\\n",
        "    .agg(sum('total_cost').alias('promotion_revenue'))\\\n",
        "    .orderBy(col('promotion_revenue').desc())\n",
        "\n",
        "promotion_performance.show()"
      ],
      "metadata": {
        "id": "9p9ng45lEE7w",
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "outputId": "5cc0da39-8d86-406f-e802-d00be90300c5"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "+--------------------+-----------------+\n",
            "|           promotion|promotion_revenue|\n",
            "+--------------------+-----------------+\n",
            "|                NONE|      17554038.81|\n",
            "|DISCOUNT ON SELEC...|      17462227.94|\n",
            "|BOGO (BUY ONE GET...|      17438953.65|\n",
            "+--------------------+-----------------+\n",
            "\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "customer_value = retails_processed \\\n",
        "    .groupBy('customer_category') \\\n",
        "    .agg(\n",
        "        count('*').alias('transaction_count'),\n",
        "        avg('total_cost').alias('avg_spend'),\n",
        "        sum('total_cost').alias('total_spend')\n",
        "    ) \\\n",
        "    .orderBy(col('total_spend').desc())\n",
        "customer_value.show()"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "id": "60e27czOURzJ",
        "outputId": "eb771679-1724-4f68-ab92-ba1e161a4fe9"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "+-----------------+-----------------+---------+-----------+\n",
            "|customer_category|transaction_count|avg_spend|total_spend|\n",
            "+-----------------+-----------------+---------+-----------+\n",
            "|         TEENAGER|           125319|52.529091| 6582893.18|\n",
            "|        HOMEMAKER|           125418|52.461417| 6579605.97|\n",
            "|   SENIOR CITIZEN|           125485|52.342672| 6568220.19|\n",
            "|          RETIREE|           125072|52.435589| 6558223.95|\n",
            "|          STUDENT|           124842|52.487994| 6552706.16|\n",
            "|     PROFESSIONAL|           124651|52.525762| 6547388.77|\n",
            "|      YOUNG ADULT|           124577|52.448246| 6533845.14|\n",
            "|      MIDDLE-AGED|           124636|52.411318| 6532337.04|\n",
            "+-----------------+-----------------+---------+-----------+\n",
            "\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "payment_usage = retails_processed\\\n",
        "    .groupBy('payment_method')\\\n",
        "    .count()\\\n",
        "    .orderBy(col('count').desc())\n",
        "payment_usage.show()"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "id": "pgDyBfp6Vsiv",
        "outputId": "6b0e260a-e87a-435a-86d7-c84740fa2f97"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "+--------------+------+\n",
            "|payment_method| count|\n",
            "+--------------+------+\n",
            "|          CASH|250230|\n",
            "|    DEBIT CARD|250074|\n",
            "|   CREDIT CARD|249985|\n",
            "|MOBILE PAYMENT|249711|\n",
            "+--------------+------+\n",
            "\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "season_performance = retails_processed\\\n",
        "    .groupBy('season')\\\n",
        "    .agg(sum('total_cost').alias('season_revenue'))\\\n",
        "    .orderBy(col('season_revenue').desc())\n",
        "season_performance.show()"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "id": "TDEDv9ySW2-o",
        "outputId": "ff4b1fc4-dd7d-4b67-c1ac-deeefb92e333"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "+------+--------------+\n",
            "|season|season_revenue|\n",
            "+------+--------------+\n",
            "|  FALL|   13136913.71|\n",
            "|SUMMER|   13116675.79|\n",
            "|SPRING|   13113238.75|\n",
            "|WINTER|   13088392.15|\n",
            "+------+--------------+\n",
            "\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "discount_impact = retails_processed\\\n",
        "    .groupBy('discount_applied')\\\n",
        "    .agg(count('*').alias('transaction_count'),\n",
        "        sum('total_cost').alias('total_revenue'),\n",
        "        avg('total_cost').alias('avg_transaction_value'))\n",
        "discount_impact.show()"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "id": "MWyKoUzIXYqb",
        "outputId": "89132f5c-0be9-4526-deba-2da02337f742"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "+----------------+-----------------+-------------+---------------------+\n",
            "|discount_applied|transaction_count|total_revenue|avg_transaction_value|\n",
            "+----------------+-----------------+-------------+---------------------+\n",
            "|            true|           500104|  26248916.26|            52.486915|\n",
            "|           false|           499896|  26206304.14|            52.423512|\n",
            "+----------------+-----------------+-------------+---------------------+\n",
            "\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "top_product = retails_processed\\\n",
        ".groupBy('product')\\\n",
        ".agg(sum('total_cost').alias('revenue'))\\\n",
        ".orderBy(col('revenue').desc())\\\n",
        ".limit(10)\n",
        "\n",
        "top_product.show()"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "id": "30nPNSotq8gU",
        "outputId": "c4081096-d26d-44e5-dede-7d5ac5f71b19"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "+--------------+---------+\n",
            "|       product|  revenue|\n",
            "+--------------+---------+\n",
            "|['Toothpaste']|253689.01|\n",
            "|     ['Honey']|135409.85|\n",
            "| ['Deodorant']|133082.77|\n",
            "|     ['Pasta']|131765.69|\n",
            "|      ['Eggs']|131565.71|\n",
            "|   ['Vinegar']|131538.39|\n",
            "|      ['Soap']|131183.28|\n",
            "|    ['Razors']|130956.57|\n",
            "|    ['Salmon']|130940.12|\n",
            "|       ['Jam']|130617.22|\n",
            "+--------------+---------+\n",
            "\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "customer_buying_metrics = retails_processed\\\n",
        ".groupBy('customer_category')\\\n",
        "  .agg(avg('total_items').alias('avg_items_per_transaction'))\n",
        "\n",
        "customer_buying_metrics.show()"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "id": "2_AnB08nsGLJ",
        "outputId": "582634f1-9227-4650-9ed9-ed91f849e3a0"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "+-----------------+-------------------------+\n",
            "|customer_category|avg_items_per_transaction|\n",
            "+-----------------+-------------------------+\n",
            "|         TEENAGER|        5.489670361238121|\n",
            "|          RETIREE|         5.50546085454778|\n",
            "|      YOUNG ADULT|       5.4955489376048545|\n",
            "|      MIDDLE-AGED|        5.501388041978241|\n",
            "|   SENIOR CITIZEN|        5.493525122524605|\n",
            "|          STUDENT|        5.496787939956104|\n",
            "|        HOMEMAKER|       5.5024637611826055|\n",
            "|     PROFESSIONAL|       5.4826595855628915|\n",
            "+-----------------+-------------------------+\n",
            "\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "from pyspark.sql.functions import hour\n",
        "\n",
        "peak_buying_hour = retails_processed\\\n",
        ".withColumn('hour', hour('transaction_time'))\\\n",
        "  .groupBy('hour')\\\n",
        "  .agg(count('*').alias('transactions'))\\\n",
        "  .orderBy('hour')\n",
        "\n",
        "peak_buying_hour.show()"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "id": "sB-jymVosr1j",
        "outputId": "dc7a9bd1-2304-4ad4-bc49-2330ab79a47d"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "+----+------------+\n",
            "|hour|transactions|\n",
            "+----+------------+\n",
            "|   0|       41416|\n",
            "|   1|       41224|\n",
            "|   2|       41617|\n",
            "|   3|       41917|\n",
            "|   4|       41768|\n",
            "|   5|       41574|\n",
            "|   6|       41508|\n",
            "|   7|       41521|\n",
            "|   8|       41926|\n",
            "|   9|       41841|\n",
            "|  10|       42021|\n",
            "|  11|       41154|\n",
            "|  12|       41790|\n",
            "|  13|       41817|\n",
            "|  14|       41739|\n",
            "|  15|       41819|\n",
            "|  16|       41505|\n",
            "|  17|       41655|\n",
            "|  18|       41843|\n",
            "|  19|       41646|\n",
            "+----+------------+\n",
            "only showing top 20 rows\n"
          ]
        }
      ]
    },
    {
      "cell_type": "code",
      "source": [
        "from pyspark.sql.functions import when\n",
        "\n",
        "discount_percentage_revenue = retails_processed\\\n",
        ".agg(\n",
        "    (sum(when(col('discount_applied') == True, col('total_cost')))/\n",
        "     sum(col('total_cost')) * 100).alias('discounted_revenue_pct'))\n",
        "\n",
        "discount_percentage_revenue.show()"
      ],
      "metadata": {
        "colab": {
          "base_uri": "https://localhost:8080/"
        },
        "id": "NwY9suWDs-Vr",
        "outputId": "0e268d23-3b9b-42b8-ea81-34a08d460963"
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "+----------------------+\n",
            "|discounted_revenue_pct|\n",
            "+----------------------+\n",
            "|       50.040617616011|\n",
            "+----------------------+\n",
            "\n"
          ]
        }
      ]
    }
  ]
}